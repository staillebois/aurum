...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs shadow-lg">
      ...
      <div className="mt-1 text-zinc-400">Score: {d.opportunityScore}</div>
      <div className="text-zinc-400">MRR: ${d.estimatedMrr?.toLocaleString()}</div>
    </div>
  )
}

export default function ScoreVsMrrChart({ data }: Props) {
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-4">
      <h3 className="mb-4 text-sm font-medium text-zinc-400">Opportunity Score vs MRR</h3>
      <ResponsiveContainer width="100%" height={350}>
        <ScatterChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#27272a" />
          <XAxis type="number" dataKey="opportunityScore" name="Score" tick={{ fill: '#a1a1aa', fontSize: 11 }} domain={[0, 100]} />
